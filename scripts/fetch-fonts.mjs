...
//
// Kept: families referenced by the site stylesheets (Inter, Montserrat, Open Sans, Poppins) and the
// latin + latin-ext subsets (every glyph on every page is in latin; latin-ext kept as a safety net).
// Dropped: ABeeZee / Comfortaa (no stylesheet references them — the export only downloaded them
// because WebFont loader probes every family) and cyrillic/greek/vietnamese subsets (no such glyphs).
...
const KEEP_FAMILIES = new Set(['Inter', 'Montserrat', 'Open Sans', 'Poppins'])
...
for (const [, subset, body] of blocks) {
  ...
  if (!KEEP_FAMILIES.has(family) || !KEEP_SUBSETS.has(subset)) continue
